...
inline usize hm_fn_hash(const char* key) {
    assert(key);
...
inline usize hm_fn_hash(usize key) {
    return key;
}

inline bool hm_fn_eq(const char* k1, const char* k2) {
    return strcmp(k1, k2) == 0;
}

inline bool hm_fn_eq(usize k1, usize k2) {
    return k1 == k2;
}
...
// Functions that can't be in shared header file
#ifdef PS_HH_IMPLEMENTATION

// Debug allocator implementation
#ifdef PS_HH_LEAK_CHECK
struct _mem_entry {
    usize ptr;
    usize size;
    bool freed;
...
void mem_leak_check() {
    ...
    for (usize i = 0; i < _mem_entry_size; i++) {
        ...
        while (entry) {
            if (!entry->freed) {
                printf("(%s:%d) Leaked %llu bytes. (%p)\n", entry->file, entry->line, entry->size, (void*)entry->ptr);
                ++leaks;
            }
    ...
    if (!leaks) printf("No memory leaks detected.\n");
}
#else
void mem_leak_check() {}  // Memory leak checking is disabled on release builds
#endif
